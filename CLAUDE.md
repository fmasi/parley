...
22. **Engine model downloads:** FluidAudio downloads its Parakeet model (~500MB) eagerly when the user clicks Save in Settings — never at recording start/end. Use `FluidAudioEngine.isModelCached()` to check cache state; `FluidAudioEngine.preDownloadModel(progress:)` to trigger download-only (no in-memory load). SpeechAnalyzer uses the system framework (no download).
